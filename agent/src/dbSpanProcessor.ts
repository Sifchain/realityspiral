...
const postgresUrl = process.env.POSTGRES_URL || 'postgresql://softwareengineer-frontend:mysecretpassword@localhost:5432/tracing_database';
...
async function insertTrace(spanData: any): Promise<void> {
  if (pgClient) {
    ...
    try {
      ...
      elizaLogger.info('✅ Span inserted successfully into PostgreSQL:', spanData.span_name);
    } catch (error) {
...
export class DBSpanProcessor implements SpanProcessor {
  ...
  onEnd(span: ReadableSpan): void {
    ...
    const spanData = {
      ...
      links: span.links || [],
      resource: resource,
      agent_id: safeTrim(attributes['agent.id']) || safeTrim(attributes.agentId),
      session_id: safeTrim(attributes['session.id']) || safeTrim(attributes.sessionId),
