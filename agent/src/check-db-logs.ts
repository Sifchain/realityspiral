...
const postgresUrl = process.env.POSTGRES_URL || 'postgresql://user@localhost:5432/tracing_database';
...
async function checkTraceLogs() {
  ...
  try {
    ...
    if (tableExists.rows[0].exists) {
      ...
      elizaLogger.info(`Found ${traces.rows.length} trace records in the database.`);
      
      // Display the traces
      if (traces.rows.length > 0) {
        traces.rows.forEach((trace, index) => {
          const timestamp = new Date(trace.start_time).toISOString();
          elizaLogger.info(`Trace ${index + 1}: ${trace.span_name} - Session: ${trace.session_id || 'unknown'} - Agent: ${trace.agent_id || 'unknown'} - Time: ${timestamp}`);
        });
      } else {
        elizaLogger.warn('No traces found in the database.');
      }
    } else {
      elizaLogger.error('❌ Traces table does not exist in the PostgreSQL database');
    }
  } catch (error) {
