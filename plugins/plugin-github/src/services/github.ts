...
export class GitHubService {
	...
	async getDocumentation(docPath = ""): Promise<string[]> {
		try {
			const response = await this.octokit.repos.getContent({
				owner: this.config.owner,
				repo: this.config.repo,
		...
		try {
			...
			if (Array.isArray(response.data)) {
				return response.data
					.filter(
						(file) =>
							file.type === "file" && !file.name.toLowerCase().includes("test"),
					)
					.map((file) => file.path);
			}
			return [];
	...
	): Promise<RestEndpointMethodTypes["issues"]["update"]["response"]["data"]> {
		try {
	...
	public async getPRDiffText(url: string): Promise<string> {
		try {
			...
			const diffSections = (diffResponse.data as string).split("diff --git");
			...
			const truncatedSections = diffSections.map((section) => {
				...
				if (processedSection.length > 1000) {
					return `${processedSection.substring(0, 1000)}\n...[diff truncated]...`;
				}
	...
	public getPositionFromDiff(
		...
		for (let i = 0; i < diffLines.length; i++) {
			...
			if (line.startsWith("@@")) {
				...
				if (hunkMatch) {
					currentLineInFile = Number.parseInt(hunkMatch[1], 10) - 1;
				}
				continue;
			}
	...
	async fetchLabelIds(labelNames: string[]): Promise<string[]> {
		...
		try {
			const { repository }: GraphQlQueryResponseData = await this.graphqlClient(
				query,
				{
					owner: this.config.owner,
					repo: this.config.repo,
			...
			const labelMap = new Map(
				repository.labels.nodes.map((label: { id: string; name: string }) => [
					label.name,
					label.id,
				]),
			);
