...
describe("NodeJSCache", () => {
	...
	it("set keys with ttl", async () => {
		...
		expect(Math.floor((underlyingCache.getTtl("key1") ?? 0) / 1000)).toEqual(
			currentEpochTimeInSeconds + 600,
		);
		expect(Math.floor((underlyingCache.getTtl("key2") ?? 0) / 1000)).toEqual(
			currentEpochTimeInSeconds + 10,
		);
	});
});
