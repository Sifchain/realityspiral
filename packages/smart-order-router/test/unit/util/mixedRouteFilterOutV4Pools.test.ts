...
import { computeAllMixedRoutes } from "../../../src/routers/alpha-router/functions/compute-all-routes";
import { mixedRouteFilterOutV4Pools } from "../../../src/util/mixedRouteFilterOutV4Pools";
import {
	DAI_USDT,
	DAI_USDT_LOW,
	DAI_USDT_V4_LOW,
	...
	it("filter out v4 pool mixed route", async () => {
		...
		expect(
			filteredRoutes.every(
				(route) => !route.pools.some((pool) => pool instanceof V4Pool),
			),
		).toBeTruthy();
	});
});
