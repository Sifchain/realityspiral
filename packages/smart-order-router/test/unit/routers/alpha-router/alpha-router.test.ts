...
import type { RouteWithValidQuote } from "../../../../build/main/routers/alpha-router/entities/route-with-valid-quote";
import {
	AlphaRouter,
	...
	type IGasModel,
	INTENT,
	type IV3PoolProvider,
	type IV4PoolProvider,
	MixedRoute,
	MixedRouteWithValidQuote,
	OnChainQuoteProvider,
	type RouteWithQuotes,
	type SupportedExactOutRoutes,
	type SupportedRoutes,
	type SwapAndAddConfig,
	type SwapAndAddOptions,
	type SwapOptions,
	SwapRouterProvider,
	SwapToRatioStatus,
	SwapType,
	type TokenPropertiesMap,
...
import type { ProviderConfig } from "../../../../src/providers/provider";
import {
	TokenValidationResult,
	TokenValidatorProvider,
} from "../../../../src/providers/token-validator-provider";
import {
	type IV2PoolProvider,
	V2PoolProvider,
} from "../../../../src/providers/v2/pool-provider";
...
describe("alpha router", () => {
	...
	class TestAlphaRouter extends AlphaRouter {
		...
		public static refreshMixedPools(
			...
			return super.refreshMixedPools(
				route,
				routingConfig,
				v2PoolProvider,
				v3PoolProvider,
	...
	describe("isAllowedToEnterCachedRoutes", () => {
		...
		test("returns correct values based on random percentage and hooksOptions", () => {
			...
			expect(
				AlphaRouter.isAllowedToEnterCachedRoutes(INTENT.QUOTE, undefined),
			).toBe(true);
			expect(
				AlphaRouter.isAllowedToEnterCachedRoutes(undefined, undefined),
			).toBe(true);
		});

		test("returns correct values based on random percentage and swapRouter", () => {
			expect(
				AlphaRouter.isAllowedToEnterCachedRoutes(INTENT.QUOTE, undefined, true),
			).toBe(true);
			expect(
				AlphaRouter.isAllowedToEnterCachedRoutes(
					INTENT.QUOTE,
	...
	describe("refreshPools tests", () => {
		...
		test("V2 pools refresh", async () => {
			const route = new V2Route([DAI_USDT_V2_OLD, DAI_USDT_V2_OLD], DAI, USDT);
			...
			localV2PoolProvider.getPools.resolves(
				buildMockV2PoolAccessor(v2MockPools),
			);

			const refreshedRoute = await TestAlphaRouter.refreshV2Pools(
				route,
				ROUTING_CONFIG,
				localV2PoolProvider,
			);
		...
		test("V3 pools refresh", async () => {
			...
			expect(route.pools).toEqual([
				USDT_DAI_V3_MEDIUM_OLD,
				USDT_DAI_V3_MEDIUM_OLD,
			]);

			const v3MockPools = [USDT_DAI_V3_MEDIUM_NEW];
			const localV3PoolProvider = sinon.createStubInstance(V3PoolProvider);
			localV3PoolProvider.getPools.resolves(
				buildMockV3PoolAccessor(v3MockPools),
			);

			const refreshedRoute = await TestAlphaRouter.refreshV3Pools(
				route,
				ROUTING_CONFIG,
				localV3PoolProvider,
			);
			expect(localV3PoolProvider.getPools.called).toBe(true);
			expect(refreshedRoute.pools).toEqual([
				USDT_DAI_V3_MEDIUM_NEW,
				USDT_DAI_V3_MEDIUM_NEW,
			]);
		});
		...
		test("V4 pools refresh", async () => {
			...
			localV4PoolProvider.getPools.resolves(
				buildMockV4PoolAccessor(v4MockPools),
			);

			const refreshedRoute = await TestAlphaRouter.refreshV4Pools(
				route,
				ROUTING_CONFIG,
				localV4PoolProvider,
			);
		...
		test("Mixed pools refresh", async () => {
			...
			const refreshedRoute = await TestAlphaRouter.refreshMixedPools(
				route,
				ROUTING_CONFIG,
				localV2PoolProvider,
				localV3PoolProvider,
		...
		test("testRefreshPools", async () => {
			...
			localV4PoolProvider.getPoolId.callsFake(
			...
			// test the method
			await testAlphaRouter.refreshPools(
				[
					v2RouteWithValidQuote,
					v3RouteWithValidQuote,
					v4RouteWithValidQuote,
			...
			expect(v2RouteWithValidQuote.route.pairs).toEqual([
				DAI_USDT_V2_NEW,
				DAI_USDT_V2_NEW,
			]);
			expect(v3RouteWithValidQuote.route.pools).toEqual([
				USDT_DAI_V3_MEDIUM_NEW,
				USDT_DAI_V3_MEDIUM_NEW,
			]);
			expect(v4RouteWithValidQuote.route.pools).toEqual([
				USDT_DAI_V4_LOW_NEW,
				USDT_DAI_V4_LOW_NEW,
			]);
			expect(mixedRouteWithValidQuote.route.pools).toEqual([
				DAI_USDT_V2_NEW,
				USDT_DAI_V3_MEDIUM_NEW,
				USDT_DAI_V4_LOW_NEW,
			]);
