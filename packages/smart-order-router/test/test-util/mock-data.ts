...
import { ChainId, type Currency, Ether, Token } from "@uniswap/sdk-core";
...
export const poolToV4SubgraphPool = (
	...
	return {
		...
		tvlETH: Number.parseFloat(pool.liquidity.toString()),
		tvlUSD: Number.parseFloat(pool.liquidity.toString()),
	};
};
	...
	return {
		...
		getPool: (tokenA, tokenB) =>
			_.find(
				pools,
				(p) =>
					Pair.getAddress(p.token0, p.token1) ==
