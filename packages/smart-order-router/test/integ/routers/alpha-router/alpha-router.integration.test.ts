...
import type { Permit2Permit } from "@uniswap/universal-router-sdk";
import { Pair } from "@uniswap/v2-sdk";
import { FeeAmount, Pool, encodeSqrtRatioX96 } from "@uniswap/v3-sdk";
import bunyan from "bunyan";
import { BigNumber, Wallet, type providers } from "ethers";
					...
					GREENLIST_TOKEN_PAIRS.forEach(([tokenIn, tokenOut]) => {
						it(`${tokenIn.symbol} -> ${tokenOut.symbol} with portion`, async () => {
							...
							await validateSwapRoute(
								...
								quoteGasAdjusted,
								tradeType,
								Number.parseFloat(quote.toFixed(0)),
								acceptableDifference,
								quoteGasAndPortionAdjusted,
								Number.parseFloat(
							...
							const checkTokenOutAmount = tokenOut.isNative
								? undefined
								: Number.parseFloat(amount.toFixed(0));
							const checkPortionAmount = Number.parseFloat(
						...
						tokenInAndTokenOut.forEach(([tokenIn, tokenOut]) => {
							it(`fee-on-transfer ${tokenIn?.symbol} -> ${tokenOut?.symbol}`, async () => {
								...
								for (const response of responses) {
									...
									if (enableFeeOnTransferFeeFetching) {
										const checkTokenInAmount = Number.parseFloat(
											amount.toFixed(0),
										);
										const checkTokenOutAmount = Number.parseFloat(
											amount.toFixed(0),
										);
	...
	for (const chain of _.filter(
		...
		for (const tradeType of [TradeType.EXACT_INPUT, TradeType.EXACT_OUTPUT]) {
			...
			describe(`${ID_TO_NETWORK_NAME(chain)} ${tradeType} 2xx`, () => {
				...
				if (isTenderlyEnvironmentSet()) {
					describe(`Simulate + Swap ${tradeType.toString()}`, () => {
						...
						it(`${wrappedNative.symbol} -> erc20`, async () => {
							...
							console.log(
								`chain ${chain} GAS_ESTIMATE_DEVIATION_PERCENT ${percentDiff.toNumber()} expected ${GAS_ESTIMATE_DEVIATION_PERCENT[chain]} ${swapWithSimulation!.estimatedGasUsed.toNumber()} ${swap!.estimatedGasUsed.toNumber()}`,
							);
							expect(
								percentDiff.lte(
						...
						it(`${native} -> erc20`, async () => {
							...
							const percentDiff = gasEstimateDiff
								...
								.div(swapWithSimulation!.estimatedGasUsed);
							console.log(
								`chain ${chain} GAS_ESTIMATE_DEVIATION_PERCENT ${percentDiff.toNumber()} expected ${GAS_ESTIMATE_DEVIATION_PERCENT[chain]} ${swapWithSimulation!.estimatedGasUsed} ${swap!.estimatedGasUsed}`,
							);
