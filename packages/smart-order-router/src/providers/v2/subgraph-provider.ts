    ...
    `;
		...
		const tracked = pools.filter(
			(pool) =>
				...
				Number.parseFloat(pool.trackedReserveETH) > this.trackedEthThreshold,
		);
		...
		const poolsSanitized: V2SubgraphPool[] = pools
			.filter((pool) => {
				return (
					...
					pool.token1.id == FEI ||
					this.isVirtualPairBaseV2Pool(pool) ||
					Number.parseFloat(pool.trackedReserveETH) >
						this.trackedEthThreshold ||
					Number.parseFloat(pool.reserveUSD) > this.untrackedUsdThreshold
				);
			})
			.map((pool) => {
				return {
					...
					supply: Number.parseFloat(pool.totalSupply),
					reserve: Number.parseFloat(pool.trackedReserveETH),
