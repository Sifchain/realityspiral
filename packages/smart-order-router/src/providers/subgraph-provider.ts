...
> {
    ...
    `;
		...
		const untrackedPools = pools.filter(
			(pool) =>
				Number.parseInt(pool.liquidity) > 0 ||
				Number.parseFloat(pool.totalValueLockedETH) >
					this.trackedEthThreshold ||
				Number.parseFloat(pool.totalValueLockedUSDUntracked) >
		...
		if (this.protocol === Protocol.V3) {
			...
			poolsSanitized = pools
				.filter(
					(pool) =>
						(Number.parseInt(pool.liquidity) > 0 &&
							Number.parseFloat(pool.totalValueLockedETH) === 0) ||
						Number.parseFloat(pool.totalValueLockedETH) >
							this.trackedEthThreshold,
				)
				.map((pool) => {
