...
import { Protocol, SwapRouter, type Trade, ZERO } from "@uniswap/router-sdk";
import {
	ChainId,
	type Currency,
	Fraction,
...
import {
	...
	type IOnChainQuoteProvider,
	type IRouteCachingProvider,
	type ISwapRouterProvider,
	type ITokenPropertiesProvider,
	type IV2QuoteProvider,
		...
		ISwapToRatio<AlphaRouterConfig, SwapAndAddConfig>
{
	...
	constructor({
		...
		if (v3SubgraphProvider) {
			...
			this.v3SubgraphProvider = new V3SubgraphProviderWithFallBacks([
				//Before we used: new StaticV3SubgraphProvider(chainId, this.v3PoolProvider),
	...
	private async getSwapRouteFromChain(
		...
		// We only support v3 now.
		const v3CandidatePoolsPromise: Promise<V3CandidatePools | undefined> =
			getV3CandidatePools({
				tokenIn,
				tokenOut,
		...
		const v3CandidatePools = await Promise.resolve(v3CandidatePoolsPromise);
		const getQuoteResult = await this.v3Quoter.getRoutesThenQuotes(
			tokenIn,
			tokenOut,
			amount,
			amounts,
	...
	private async getGasModels(
		...
		const v3GasModel = await Promise.resolve(v3GasModelPromise);
