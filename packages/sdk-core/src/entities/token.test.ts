...
describe("Token", () => {
	...
	describe("#constructor with bypassChecksum = true", () => {
		...
		it("creates the token with a valid address", () => {
			expect(
				new Token(3, ADDRESS_TWO, 18, undefined, undefined, bypassChecksum)
					.address,
			).toBe(ADDRESS_TWO);
		});
		it("fails with invalid address", () => {
		...
		it("fails with negative decimals", () => {
			expect(
				() =>
					new Token(3, ADDRESS_ONE, -1, undefined, undefined, bypassChecksum)
						.address,
			).toThrow("DECIMALS");
		});
		it("fails with 256 decimals", () => {
			expect(
				() =>
					new Token(3, ADDRESS_ONE, 256, undefined, undefined, bypassChecksum)
						.address,
			).toThrow("DECIMALS");
		});
		it("fails with non-integer decimals", () => {
			expect(
				() =>
					new Token(3, ADDRESS_ONE, 1.5, undefined, undefined, bypassChecksum)
						.address,
			).toThrow("DECIMALS");
		});
	});
