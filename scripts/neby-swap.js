...
import { Percent, Token, CurrencyAmount, TradeType, MULTICALL_ADDRESSES, ChainId } from '@uniswap/sdk-core';
...
async function main() {
    ...
    const amountIn = CurrencyAmount.fromRawAmount(ROSE, ethers.utils.parseUnits("0.1", 18));
    ...
    const { calldata, value } = SwapRouter.swapERC20CallParameters(route.trade, {
        ...
        deadlineOrPreviousBlockhash: Math.floor(Date.now() / 1000 + 1800).toString(),
    });
