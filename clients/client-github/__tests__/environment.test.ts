...
import { githubEnvSchema, validateGithubConfig } from "../src/environment";
...
describe("GitHub Environment Configuration", () => {
	...
	it("throws error for missing configuration", async () => {
		...
		vi.mocked(mockRuntime.getSetting).mockImplementation(
			(key: string) => invalidConfig[key as keyof typeof invalidConfig],
		);
	...
	it("throws error for partial configuration", async () => {
		...
		vi.mocked(mockRuntime.getSetting).mockImplementation(
			(key: string) => partialConfig[key as keyof typeof partialConfig],
		);
