	...
	return (
		<div className="flex flex-col p-6 h-[100vh]">
			...
			<div className="p-4 rounded-lg bg-[#161616] shadow-lg sticky top-0">
				<Label className="text-white text-lg font-semibold">Filters</Label>
            ...
            focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200 ease-in-out cursor-pointer"
						...
						>
							...
							{uniqueAgents.map((agent, index) => (
								...
								<option key={index} value={agent} className="text-gray-900">
									{agent}
								</option>
							))}
						</select>
					...
					<div className="flex items-end">
						...
						<button
							...
							onClick={() => fetchTraceData(selectedRoom, filters)}
			...
			<div className="flex flex-grow h-full overflow-hidden mt-4">
				...
				{selectedRoom && (
					<div className="w-2/3 p-4 rounded-lg shadow-lg overflow-y-auto h-full">
						...
						{loading && <p className="mt-4 text-white">Loading traces...</p>}
						...
						{traceData.length > 0 ? (
								...
								{traceData.map((trace, index) => (
									...
									>
										<p className="text-gray-300 font-semibold">
											Trace ID:{" "}
											<span className="text-white">{trace.trace_id}</span>
										</p>
										<p className="text-gray-300 font-semibold">
											Span Name:{" "}
											<span className="text-white">{trace.span_name}</span>
										</p>
										<p className="text-gray-300 font-semibold">
											Start Time:{" "}
										...
										<p className="text-gray-300 font-semibold">
											Duration (ms):{" "}
											<span className="text-white">{trace.duration_ms}</span>
										</p>
										<p className="text-gray-300 font-semibold">
											Agent ID:{" "}
											<span className="text-white">{trace.agent_id}</span>
										</p>
										<p className="text-gray-300 font-semibold">
											Session ID:{" "}
											<span className="text-white">{trace.session_id}</span>
										</p>
										<p className="text-gray-300 font-semibold">Attributes:</p>
							...
							<p className="mt-4 text-gray-400">No trace data available.</p>
						)}
					</div>
				)}
			</div>
